 ...
 */
public class JFrameWindowMetrics extends javax.swing.JFrame {

    /**
     * Creates new form JFrameWindow
     */
    public JFrameWindowMetrics() {
        initComponents();
        loadMetricNamesIntoList();
        setupContextMenu();
    ...
    private void loadMetricNamesIntoList() {
        java.util.Set<String> allNames = collectUniqueMetricTerms();
        java.nio.file.Path jsonRoot = findJsonRoot();
        if (jsonRoot != null) {
            aliasFilePath = jsonRoot.getParent().resolve("metric_aliases.json");
        }
        metricFilePaths.clear();
        metricFilePaths.putAll(collectMetricFilePaths());
        ...
        for (java.util.Map.Entry<String, java.util.Set<java.nio.file.Path>> entry : metricFilePaths.entrySet()) {
    ...
    private void setupContextMenu() {
        ...
        mergeItem.addActionListener(e -> mergeSelectedMetrics());
    ...
    private void mergeSelectedMetrics() {
    ...
    private java.util.Set<String> collectUniqueMetricTerms() {
        ...
        for (java.nio.file.Path path : listJsonFiles(false)) {
            try {
                ...
                terms.addAll(collectMetricTerms(new org.json.JSONObject(content)));
            } catch (Exception e) {
    ...
    private java.util.Map<String, java.util.Set<java.nio.file.Path>> collectMetricFilePaths() {
        java.util.Map<String, java.util.Set<java.nio.file.Path>> pathsByTerm = new java.util.HashMap<>();
        for (java.nio.file.Path path : listJsonFiles(false)) {
            try {
                String content = java.nio.file.Files.readString(path, java.nio.charset.StandardCharsets.UTF_8);
                java.util.Set<String> termsInFile = collectMetricTerms(new org.json.JSONObject(content));
    ...
    private java.util.Set<String> collectMetricTerms(org.json.JSONObject record) {
        java.util.Set<String> terms = new java.util.LinkedHashSet<>();
        collectMetricTerms(record, terms);
        return terms;
    }

    private void collectMetricTerms(org.json.JSONObject record, java.util.Set<String> terms) {
        Object metricsObj = record.opt("metrics_used_to_evaluate_results");
        if (metricsObj instanceof org.json.JSONArray metrics) {
            for (int i = 0; i < metrics.length(); i++) {
                addCleanTerm(terms, cleanTerm(metrics.opt(i)));
            }
        }
    }
    ...
    private void refreshList1() {
        ...
        java.util.List<String> terms = filter.isEmpty()
                ? allTerms
                : allTerms.stream().filter(t -> metricTermMatchesFilter(t, filter))
                          .collect(java.util.stream.Collectors.toList());
    ...
    private boolean metricTermMatchesFilter(String term, String filter) {
    ...
    private java.util.List<java.nio.file.Path> findMatchingMetricFiles(boolean includeNonCompliant) {
        ...
        for (java.nio.file.Path path : listJsonFiles(includeNonCompliant)) {
            try {
                String content = java.nio.file.Files.readString(path, java.nio.charset.StandardCharsets.UTF_8);
                if (recordHasMetricAlias(new org.json.JSONObject(content), aliases)) {
    ...
    private boolean recordHasMetricAlias(org.json.JSONObject record, java.util.Set<String> aliases) {
        for (String metricTerm : collectMetricTerms(record)) {
            if (containsIgnoreCase(aliases, metricTerm)) {
                return true;
            }
        }
        return false;
    ...
    private java.util.List<java.nio.file.Path> findMatchingMetricFilesForTerms(
            java.util.Collection<String> metricTerms,
            boolean includeNonCompliant) {
        java.util.Set<String> terms = new java.util.HashSet<>();
        for (String metricTerm : metricTerms) {
            String clean = cleanTerm(metricTerm);
            if (!clean.isEmpty()) {
                terms.add(clean);
            }
        }
        ...
        for (java.nio.file.Path path : listJsonFiles(includeNonCompliant)) {
            try {
                String content = java.nio.file.Files.readString(path, java.nio.charset.StandardCharsets.UTF_8);
                if (recordHasMetricAlias(new org.json.JSONObject(content), terms)) {
                    matchingFiles.add(path);
                }
            } catch (Exception ignored) {}
        }
    ...
    private java.util.Set<String> collectClusterTermsInFiles(java.util.List<java.nio.file.Path> files) {
        ...
        for (java.nio.file.Path path : files) {
            try {
                String content = java.nio.file.Files.readString(path, java.nio.charset.StandardCharsets.UTF_8);
                for (String metricTerm : collectMetricTerms(new org.json.JSONObject(content))) {
                    if (containsIgnoreCase(clusterTerms, metricTerm)) {
                        found.add(metricTerm);
                    }
                }
            } catch (Exception ignored) {}
        }
    ...
    private void openJsonViewer() {
        ...
        java.util.List<java.nio.file.Path> matchingFiles = findMatchingMetricFiles(true);
    ...
    private void showJsonWindow(String title, String content, java.nio.file.Path filePath) {
        ...
        // Default highlights: metric-related keys in green, metric values in yellow
        ...
        String[] keyTokens = {"\"metrics_used_to_evaluate_results\""};
        ...
        try {
            ...
            for (String name : collectMetricTerms(rec)) {
                String quoted = "\"" + name + "\"";
                int ni = 0;
    ...
    private int getTermFileCount(String term) {
        ...
        for (String alias : aliases) {
            java.util.Set<java.nio.file.Path> aliasFiles = metricFilePaths.get(alias);
            if (aliasFiles != null) {
    ...
    private void copyDoiToClipboard() {
        if (currentSelectedTerm == null || !aliasData.containsKey(currentSelectedTerm)) return;
        java.util.List<java.nio.file.Path> matches = findMatchingMetricFiles(false);
    ...
    private void toggleNonCompliant() {
        ...
        if (nonCompliantTerms.contains(currentSelectedTerm)) {
            ...
            moveCurrentMetricFilesToNonCompliant();
        }
    }

    private void moveCurrentMetricFilesToNonCompliant() {
        java.util.List<String> aliases = aliasData.getOrDefault(currentSelectedTerm, java.util.Collections.emptyList());
        java.util.List<String> termsToMove = chooseMetricTermsToMove(aliases);
        if (termsToMove.isEmpty()) {
            return;
        }

        java.util.List<java.nio.file.Path> matchingFiles = findMatchingMetricFilesForTerms(termsToMove, false);
        if (matchingFiles.isEmpty()) {
    ...
    private java.util.List<String> chooseMetricTermsToMove(java.util.List<String> aliases) {
        if (aliases.size() <= 1) {
    ...
    private void moveSelectedAliasesToNonCompliant() {
        ...
        java.util.List<java.nio.file.Path> matchingFiles = findMatchingMetricFilesForTerms(selectedAliases, false);
        if (matchingFiles.isEmpty()) {
            javax.swing.JOptionPane.showMessageDialog(this,
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JFrameWindowMetrics.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new JFrameWindowMetrics().setVisible(true);
            }
        });
    }
    ...
    private final java.util.Map<String, java.util.Set<java.nio.file.Path>> metricFilePaths = new java.util.HashMap<>();
