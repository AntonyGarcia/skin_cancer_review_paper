...
BACKUP_ROOT = BASE_DIR / f"metric_alias_replacement_backup_{datetime.now():%Y%m%d_%H%M%S}"
REPORT_PATH = BASE_DIR / "replace_metric_aliases_report.json"
...
def replace_metric_aliases(record: object, alias_to_term: dict[str, str]) -> list[dict[str, object]]:
    ...
    for index, old_value in enumerate(metrics):
...
def main() -> int:
    ...
    for json_path in sorted(JSON_ROOT.rglob("*.json")):
        ...
        changes = replace_metric_aliases(record, alias_to_term)
        if not changes:
            continue
    ...
    print(f"Metric replacements: {total_replacements}")
    if files_changed:
