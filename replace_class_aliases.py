...
ALIASES_PATH = BASE_DIR / "class_aliases.json"
JSON_ROOT = BASE_DIR / "json"
BACKUP_ROOT = BASE_DIR / f"alias_replacement_backup_{datetime.now():%Y%m%d_%H%M%S}"
REPORT_PATH = BASE_DIR / "replace_class_aliases_report.json"
...
def replace_lesion_type_aliases(record: object, alias_to_term: dict[str, str]) -> list[dict[str, object]]:
    if not isinstance(record, dict):
        return []

    lesion_types = record.get("lesion_types_studied")
    if not isinstance(lesion_types, list):
        return []

    changes: list[dict[str, object]] = []
    for index, item in enumerate(lesion_types):
        if not isinstance(item, dict):
            continue

        old_value = item.get("lesion_type")
        if not isinstance(old_value, str):
            continue

        new_value = alias_to_term.get(old_value)
        if not new_value or new_value == old_value:
            continue

        item["lesion_type"] = new_value
        changes.append({"index": index, "from": old_value, "to": new_value})
...
def main() -> int:
    ...
    for json_path in sorted(JSON_ROOT.rglob("*.json")):
        ...
        changes = replace_lesion_type_aliases(record, alias_to_term)
        if not changes:
            continue
    ...
    print(f"Lesion type replacements: {total_replacements}")
    if files_changed:
